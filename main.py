...
def main():
    ...
    # --- MUSIC FIX ---
    # We load music once at the start.
    music_playing = False
    try:
    ...
    try:
        ...
        cursor_img = None
        print("Cursor image not found! Using red circle fallback.")
    ...
    # State for cursor hover needs to be accessible across the loop
    is_hovering_tap = False

    while True:
        mouse_pos = pygame.mouse.get_pos()
        # Assume we aren't hovering until proven otherwise
        is_hovering_tap = False 
        ...
        if state == "MENU":
            ...
            for name, btn in item_buttons.items():
                ...
                if owned > 0: btn.text = f"{name}: SOLD OUT"; btn.color = (200, 200, 200) 
                else: btn.text = f"{name}: {price} M"; btn.color = (255, 255, 255)
                btn.draw(screen, font)

        # --- DRAW CUSTOM CURSOR AT THE ABSOLUTE END ---
        if state == "GAME" and is_hovering_tap:
            if cursor_img:
                screen.blit(cursor_img, (mouse_pos[0] - 25, mouse_pos[1] - 25))
            else:
                # Fallback: draw a red circle if the image isn't loading
